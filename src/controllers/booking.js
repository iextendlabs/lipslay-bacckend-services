const { TimeSlot, StaffZone, StaffHoliday, LongHoliday, Order, Staff, Service, ServiceCategory, StaffGeneralHoliday, Holiday } = require('../models');
const { Op } = require('sequelize');
const moment = require('moment');
const urls = require('../config/urls');
const ShortHoliday = require('../models/ShortHoliday');

// TODO remove group dependency and review the slot search function
const getBookingSlots = async (req, res) => {
  try {
    ...
    if (!services || !Array.isArray(services)) {
      return res.status(400).json({ error: "Please provide services as an array." });
    }
    if (!date) {
    ...
    if (generalHoliday) {
      return res.json({ slots: [], message: "There is a holiday and no staff available." });
    }

    // 3. Find staff zone
    const staffZone = await StaffZone.findOne({ where: { name: area } });
    if (!staffZone) {
      return res.json({ slots: [], message: "No staff zone found for the specified area." });
    }
    ...
    if (zoneStaffIds.length === 0) {
      return res.json({ slots: [], message: "No staff available in the selected area." });
    }
    ...
    for (const service of serviceWithCategories) {
      for (const category of (service.ServiceCategories || [])) {
        staffIdsFromCategories.push(...(category.Staffs || []).map(staff => staff.id));
      }
    }

    const eligibleStaffIdsFromServices = [...new Set([...staffIdsDirect, ...staffIdsFromCategories])];

    // 5. Filter staff based on area
    let eligibleZoneStaffIds = eligibleStaffIdsFromServices.filter(id => zoneStaffIds.includes(id));

    // 6. Remove staff on leave (short/long/general)
    const staffOnDateLeave = await StaffHoliday.findAll({ where: { date }, attributes: ['staff_id'] });
    ...
    const availableStaffIds = eligibleZoneStaffIds.filter(id => !unavailableStaffIds.includes(id));

    if (availableStaffIds.length === 0) {
      return res.json({ slots: [], message: "No staff available for the selected criteria." });
    }

    // 7. Get available time slots assigned to available staff
    const slotStaffRelations = await require('../models/TimeSlotToStaff').findAll({
    ...
    if (availableTimeSlotIds.length === 0) {
      return res.json({ slots: [], message: "No time slots available for the selected criteria." });
    }
    ...
    if (date === currentDate && !isAdmin) {
      timeSlotWhere = {
        ...
        [Op.or]: [
          ...
          { type: 'Partner', time_start: { [Op.lte]: currentTime }, time_end: { [Op.gte]: currentTime } },
    ...
    for (const slot of availableTimeSlots) {
      ...
      let slotStaffIds = slotStaffs.map(s => s.id).filter(id => availableStaffIds.includes(id));
      ...
      const totalAvailableSeats = (slotStaffIds.length * seatPerSlot) - orderCount;
